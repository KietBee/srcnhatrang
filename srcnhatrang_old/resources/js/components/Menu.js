export default class Menu {
  constructor() {
    ...
    this.li = this.$this.find('.main-menu-ul >li>button')
    ...
    this.isclosingmenu = 'is-closing-menu'
  ...
  microOpenCloseLevel1(currentElement, openClass, isLiLv1 = false) {
    ...
    if ($(window).width() < 992) {
      if (eleParent.find('ul').length && !eleParent.hasClass(openClass)) {
        ...
        this.liLevel1.find('> button').each((_i, el) => {
  ...
  openMainMenu() {
    this.$header.on('click', '.hamburger-menu', (e) => {
      ...
      if ($(ele).hasClass(this.isopenmenu)) {
        ...
        $(this.elementItem).addClass(this.isclosingmenu)
        ...
        this.liLevel1.removeClass('is-open-child')
      } else {
        $(this.elementItem).addClass(this.isopenmenu)
        $(this.elementItem).removeClass(this.isclosingmenu)
  ...
  closeAllSubmenu() {
    ...
    if ($liHover.hasClass('has-sub')) {
      $liHover.find('> button').attr(this.ariaExpanded, false)
    }
  }
      ...
      if ($(window).outerWidth() > 991 && liHover.length < 2) {
        ...
        timeout = setTimeout(() => {
          ...
          if ($(ele).hasClass('has-sub')) {
            $(ele).find('> button').attr(this.ariaExpanded, true)
          }
        }, 0)
      }
    })
