export class DatePickerDialog {
  ...
  getDayFromDataDateAttribute(domNode) {
    ...
    return new Date(parts[0], parseInt(parts[1], 10) - 1, parts[2], 10)
  }
  // Textbox methods
  ...
  setTextboxDate(domNode) {
    ...
    if (domNode) {
      ...
      this.days.forEach((day) =>
        day === domNode ? day.setAttribute(this.ariaSelected, 'true') : day.removeAttribute(this.ariaSelected),
      )
    }
  ...
  getDateFromTextbox() {
    ...
    if (parts.length === 3 && Number.isInteger(month) && Number.isInteger(day) && Number.isInteger(year)) {
      if (year < 100) {
        year = 2000 + year
      }
  ...
  setDateForButtonLabel() {
    ...
    if (
      ...
      const day = new Date(parseInt(parts[2], 10), parseInt(parts[0], 10) - 1, parseInt(parts[1], 10))
