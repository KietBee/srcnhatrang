<header id="headerAdmin" class="header-admin sticky top-0 z-999 flex w-full bg-white shadow-md dark:bg-slate-600 duration-500 dark:drop-shadow-none">
  <div class="container flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
    <div class="flex items-center gap-2 sm:gap-4 xl:hidden">
      <!-- Hamburger Toggle BTN -->
      <div class="pt-4 text-center relative xl:hidden">
        <button id="open-sidebar" class="border-0 hamburger-menu bg-slate-400" type="button" data-toggle="collapse" data-target="#main-sidebar"
    ...
    <div class="hidden sm:block">
      <form action="https://formbold.com/s/unique_form_id" method="POST">
        <div class="relative">
          <button class="absolute left-0 top-22p">
            <span class="icomoon icon-icon-search"></span>
          </button>

          <input type="text" placeholder="Type to search..." class="w-full pl-11 pr-4 border-0 border-b-1 border-white border-solid outline-none bg-transparent search-btn">
        </div>
      </form>
    </div>

    <div class="flex items-center gap-15 2xsm:gap-7">
      ...
      <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
        <a class="flex items-center gap-8" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
          ...
          <span class="h-24 w-24 rounded-full">
            <img src="{{ asset('images/user.jpg') }}" class="w-full h-full" alt="User">
          </span>
        ...
        <!-- Dropdown Start -->
        <div x-show="dropdownOpen" class="absolute right-0 mt-6 flex w-100 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <ul class="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark last-mb-none list-none">
            <li>
