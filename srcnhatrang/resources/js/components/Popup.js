...
export default class Popup {
  ...
  popupVideo(ele) {
    ...
    this.iframe = `<div class="popup-inner popup-video ">
              ...
              <button type="button" class="ada-back"><span class="sr-only">Back focus</span></button>
            </div>`
    this.renderPopup()
  ...
  clickOutSite() {
    ...
    $(window).keydown((e) => {
      if (
        (e.keyCode === 27 && $(this.clsPopupShow).length) ||
        (e.keyCode === 13 && $(e.target).hasClass('popup-is-close'))
      ) {
        e.preventDefault()
        this.closePopup()
      }
  ...
  focusPopupKeyup() {
    if ($('.ada-back:focus').length) {
      ...
      } else if ($(this.clsPopupShow).find(this.$ClosePopup).is(':visible')) {
  ...
  focusPopupKeydown(event) {
    ...
    if ($(this.clsPopupShow).find(this.nextFocus).length) {
      event.preventDefault()
      $(this.clsPopupShow).find('.slick-prev').focus()
    } else if ($(this.clsPopupShow).find('.slick-prev:focus').length) {
      event.preventDefault()
      $(this.clsPopupShow).find(this.$ClosePopup).eq(0).focus()
    }
    return true
