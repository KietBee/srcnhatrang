class ADAMenu {
  constructor(e) {
    ...
    this.$topLevelNodes = this.$this.find('button[aria-expanded][aria-controls]')
  ...
  init() {
    this.$topLevelNodes.each((_i, e) => {
      ...
      if (e.tagName.toLowerCase() === 'button' && e.hasAttribute('aria-controls')) {
  ...
  onMenuHasSubKeyDown(e) {
    ...
    if (e.which === 27 && $(window).outerWidth() > 991) {
      this.toggleExpand(this.openIndex, false)
    } else if (this.useArrowKeys && this.openIndex === targetMenuItemIndex && e.which === 40) {
      e.preventDefault()
      this.$controlledNodes[this.openIndex].find('button').first().focus()
  ...
  onSubMenuKeyDown(e) {
    ...
    const $menuLinks = this.$controlledNodes[this.openIndex].find('button')
  ...
  toggleMenu($menu, show) {
    if ($menu) {
      const $menuItem = $menu.parents('.menu-item').eq(0)
      if ($(window).outerWidth() > 991) {
        if (show) {
...
// Fake link behavior
$menus.each((i, _e) => {
  const $links = $menus.eq(i).find('button')
  $links.on('click', (e) => {
