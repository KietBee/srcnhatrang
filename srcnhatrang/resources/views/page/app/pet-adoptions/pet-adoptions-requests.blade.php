            ...
            <div class="grid xl:grid-cols-2 gap-6 my-10">
                <div>
                    @if (file_exists(public_path('storage/images/app/upload' . $petAdoption->image_feature)))
                        <img class="h-full w-full object-cover rounded-lg anima-left"
                ...
                <form method="post" action="{{ route('pet-adoptions-requests.createRequests') }}"
                    class="anima-right delay-2">
                    @csrf
                    <div class="mb-6">
                        <label for="pet_adoption_name" class="block mb-2 text-sm font-medium text-gray-900">Thú cưng bạn
                            muốn nhận nuôi</label>
                        <input type="hidden" name="petAdoptionID" value="{{ $petAdoption->pet_adoption_id }}" />
                        <input type="text" readonly id="pet_adoption_name" name="petAdoptionName"
                            value="{{ $petAdoption->pet->pet_name }}"
                    ...
                    <div class="mb-6">
                        <label for="reason_for_adoption" class="block mb-2 text-sm font-medium text-gray-900">Lý do nhận
                            nuôi <sup class="text-red-700 font-bold">*</sup></label>
                    ...
                    <div class="mb-6">
                        <label for="check_token" class="inline-flex items-center">
                            <input id="check_token" name="checkToken" type="checkbox"
                                class="rounded border-gray-300 text-primary-100 shadow-sm focus:ring-primary-500"
                                required="">
                            <span
                                class="ms-2 text-sm text-gray-600">{{ __('Bạn phải chịu trách nhiệm và có nghĩa vụ chăm sóc thú cưng mà mình nhận nuôi') }}<sup
                                    class="text-red-700 font-bold">*</sup></span>
                        </label>
                    </div>
                    ...
                    <div class="grid xl:grid-cols-2 gap-6 xl:gap-28">
                        <button type="submit"
                            ...
                            Gửi yêu cầu nhận nuôi <svg class="ml-2 w-6 h-6 text-white rotate-90" aria-hidden="true"
                        ...
                        <a href="{{ route('pet-adoptions') }}"
                            ...
                            Nhận nuôi thú cưng khác <svg class="ml-2 w-6 h-6 text-white" aria-hidden="true"
                                xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                viewBox="0 0 24 24">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
