    ...
    <section class="container animation">
        ...
        <div class="py-10 grid xl:grid-cols-2">
            <div class="grid gap-4">
                <div>
                    <img class="h-auto max-w-full rounded-lg anima-bottom" src="{{ $petAdoption->image_feature }}" alt="{{ $petAdoption->title }}">
                </div>
                <div class="grid grid-cols-5 gap-4">
                    @if($listImage->isNotEmpty())
                        @foreach( $listImage as $index => $image)
                            <div>
                                <img class="h-auto max-w-full rounded-lg anima-bottom delay-{{ $index + 2 }}" src="{{ $image->pet_image }}" alt="{{$petAdoption->title }}">
                            </div>
                        @endforeach
                    @else
                    @endif
            ...
            <main class="pt-8 pb-16 lg:pt-16 lg:pb-24 bg-white dark:bg-gray-900 antialiased">
                <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
                    <article
                        ...
                        @if (Route::has('login'))
                            @auth
                                <a href="{{ route('pet-adoptions-requests.requests', ['id' => $petAdoption->pet_adoption_id]) }}" class="anima-bottom delay-7 bg-gradient-to-r inline-block mt-6 from-black to-primary-300 text-white px-4 py-2 text-xl rounded-3xl font-medium focus:ring ring-black ring-opacity-10 gradient element-to-rotate">Gửi yêu cầu nhận nuôi</a>
                            @else
                            <a href="{{ route('login') }}" class="bg-gradient-to-r inline-block mt-6 from-black to-primary-300 anima-bottom delay-7 text-white px-4 py-2 text-xl rounded-3xl font-medium focus:ring ring-black ring-opacity-10 gradient element-to-rotate">Bạn phải đăng nhập để gửi yêu cầu nhận nuôi</a>
                            @endauth
                        @endif
                    ...
                    <div class="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <article class="max-w-xs anima-bottom delay-{{ $index * 2 }}">
                            <a href="{{ route('pet-adoptions.details', ['id' => $item->pet_adoption_id]) }}">
                                <img src="{{ $item->image_feature }}" class="mb-5 rounded-lg" alt="Image 1">
                            </a>
