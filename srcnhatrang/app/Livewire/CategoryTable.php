...
class CategoryTable extends LivewireTable
{
    ...
    {
        return [
            Column::make(__('ID'), 'category_id')
                ->sortable()
                ->searchable(),
            Column::make(__('Loại danh mục'), 'category_name')
                ->sortable()
                ->searchable(),
            Column::make(__('Số lượng'), 'storyCategories')
            ...
            Column::make(__('Tùy chọn'), function (Model $model): string {
                $editUrl = route('admin.category.edit', ['id' => $model->getKey()]);
