...
class StoryTable extends LivewireTable
{
    ...
    {
        return [
            ImageColumn::make(__('Ảnh đại diện'), 'feature_image_url')
            ->displayUsing(function ($feature_image_url) {
                if ($feature_image_url && file_exists(public_path('storage/images/app/upload/' . $feature_image_url))) {
                    $image = asset('storage/images/app/upload/' . $feature_image_url);
                } else {
                    $image = asset('storage/images/default.jpg');
                };
                return $image;
            }),
            Column::make(__('ID'), 'story_id')
                ->sortable()
                ->searchable(),
            SelectColumn::make(__('Trạng thái kiểm duyệt'), 'is_approved')
                ->options([
                    '1' => 'Đã duyệt',
                    '0' => 'Chờ duyệt',
                ])
                ->displayUsing(function ($value, $model) {
                    ...
                    '<a href="'.route('admin.story.update', ['id' => $model->getKey()]).'" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">Chờ kiểm duyệt</a>';
                })                
                ->sortable()
                ->searchable()
                ->asHtml(),   
            Column::make(__('Tiêu đề'), 'title')
                ...
                ->sortable()
                ->searchable(),
            Column::make(__('Nội dung'), 'content')
                ->qualifyUsingAlias()
                ->sortable()
                ->searchable(),
            Column::make(__('Tác giả'), 'author_id')
                ->sortable()
                ->searchable(),
            Column::make(__('Người kiểm duyệt'), 'approver_id')
                ->sortable()
                ->searchable(),
            ...
            DateColumn::make(__('Ngày kiểm duyệt'), 'approved_at')
                ...
                ->searchable()
                ->format('H:i:s d/m/Y'),
            SelectColumn::make(__('Trạng thái chỉnh sửa'), 'is_edited')
                ->options([
                    '1' => 'Đã chỉnh sửa',
                    '0' => 'Đăng lần đầu',
                ]),
            DateColumn::make(__('Ngày tạo bài'), 'created_at')
                ->sortable()
                ->searchable()
                ->format('H:i:s d/m/Y'),
