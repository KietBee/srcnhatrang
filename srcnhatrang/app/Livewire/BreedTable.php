...
class BreedTable extends LivewireTable
{
    ...
    {
        return [
            Column::make(__('ID'), 'breed_id')
                ->sortable()
                ->searchable(),
            SelectColumn::make(__('Loại'), 'specie_id')
                ->options(
                    Specie::query()->pluck('specie_name', 'specie_id')->toArray()
                )
                ->displayUsing(function ($value) {
                    $specie = Specie::find($value);
                    return $specie ? $specie->specie_name : '';
                })
                ->sortable()
                ->searchable(),
            Column::make(__('Loại tuổi'), 'breed_name')
                ->sortable()
                ->searchable(),
            ...
            Column::make(__('Tùy chọn'), function (Model $model): string {
                $editUrl = route('admin.breed.edit', ['id' => $model->getKey()]);
