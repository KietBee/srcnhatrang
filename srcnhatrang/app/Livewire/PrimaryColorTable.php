...
class PrimaryColorTable extends LivewireTable
{

    protected string $model = PrimaryColor::class;
    protected bool $deferLoading = true;
    protected bool $useSelection = false;
    protected function columns(): array
    {
        return [
            Column::make(__('ID'), 'primary_color_id')
                ->sortable()
                ->searchable(),
            Column::make(__('Màu sắc'), 'primary_color_name')
                ->sortable()
                ->searchable(),
            ...
            Column::make(__('Tùy chọn'), function (Model $model): string {
                $editUrl = route('admin.primary-color.edit', ['id' => $model->getKey()]);
