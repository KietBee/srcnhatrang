...
class UserTypeTable extends LivewireTable
{
    ...
    {
        return [
            Column::make(__('ID'), 'user_type_id')
                ->sortable()
                ->searchable(),
            Column::make(__('Loại người dùng'), 'user_type_name')
                ->sortable()
                ->searchable(),
            Column::make(__('Số lượng'), 'users')
            ...
            Column::make(__('Tùy chọn'), function (Model $model): string {
                $editUrl = route('admin.user-type.edit', ['id' => $model->getKey()]);
