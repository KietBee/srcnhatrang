...
class SpecieTable extends LivewireTable
{

    protected string $model = Specie::class;
    protected bool $deferLoading = true;
    protected bool $useSelection = false;
    protected function columns(): array
    {
        return [
            Column::make(__('ID'), 'specie_id')
                ->sortable()
                ->searchable(),
            Column::make(__('Loại thú cưng'), 'specie_name')
                ->sortable()
                ->searchable(),
            Column::make(__('Số lượng'), 'breed')
            ...
            Column::make(__('Tùy chọn'), function (Model $model): string {
                $editUrl = route('admin.species.edit', ['id' => $model->getKey()]);
